...
std::vector<torch::Tensor> pool_backward(
    ...
    auto max_val = torch::zeros({batch, channel, width}, torch::dtype(torch::kFloat).device(torch::kCUDA));
    auto max_ind = torch::zeros({batch, channel, width}, torch::dtype(torch::kLong).device(torch::kCUDA));
    ...
    auto gt_mask = torch::zeros({batch, channel, width}, torch::dtype(torch::kByte).device(torch::kCUDA));
    auto max_temp = torch::zeros({batch, channel, width}, torch::dtype(torch::kFloat).device(torch::kCUDA));
